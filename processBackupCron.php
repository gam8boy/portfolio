...
function processBackup()
{
    ...
    if(file_exists(BACKUP_DIR.$zipFilename)):
        $model->getDb()->insert("backup", array("filename" => $zipFilename, "schoolyear" => $model->currentSchoolYear(), "backup_date" => date("Y-m-d H:i:s")));
    endif;
}

$model = new Model();
$checkBackup = $model->getDb()->selectSingleData("SELECT * FROM backup WHERE DATE(backup_date) = '".date('Y-m-d')."'");
if($checkBackup == null):
    processBackup();
endif;
